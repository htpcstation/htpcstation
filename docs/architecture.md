...
- **`getMovieGenres`/`getShowGenres` block the main thread** — These are synchronous `@Slot` methods that call `self._client.get_genres()`. When the server is unreachable, `_get()` retries 3 times with 1s + 3s backoff ≈ 4s UI freeze. Guard with `if not self._available: return []`.
